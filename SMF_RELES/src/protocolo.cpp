...
int protocolo_Extraer_PAYLOAD (String pakete, char *mensaje, int lenght_payload, int total_length) {
  ...
  /*if (pakete.indexOf ("#VINCULAR*") >= 0) {
    Serial.println (F("---- MENSAJE: VINCULAR -----"));
    return LORA_VINCULAR;
  } else if (pakete.indexOf ("#PING*") >= 0) {
    Serial.println (F("---- MENSAJE: PING -----"));
    return LORA_PING;
  } else if (pakete.indexOf ("#ACK*") >= 0) {
    Serial.println (F("---- MENSAJE: ACK -----"));
    return LORA_ACK;*/
